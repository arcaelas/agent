...
- `user` and `tool` content accepts `string | ContentBlock[]` (multimodal)
- `system` and `assistant` content is `string | null` (plain text only)
...
// where MessageContent = string | ContentBlock[]
```
